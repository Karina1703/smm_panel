...
export async function POST(request) {
  ...
  console.log("================== FREEKASSA POSTBACK START ==================");
  ...
  try {
    ...
    const sign = md5(`${req.MERCHANT_ID}:${req.AMOUNT}:${process.env.FREEKASSA_SECRET2}:${req.MERCHANT_ORDER_ID}`);
    ...
    if (sign === req.SIGN && payment.status === 0 && payment.type === "Add Funds") {
      console.log("====== NEW BALANCE TOP UP ======");
      console.log(req);

      await creditUserBalance(payment);

      console.log("================== FREEKASSA POSTBACK END ==================");
